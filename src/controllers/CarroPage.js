...
export default class CarroController {
  ...
  static async getCarroModel (req, res) {
    ...
    try {
      ...
      if(!carroModel.length) return res.status(404).json({mensagem: "Carro de modelo inexistente."})
  ...
  static async getCarro (req, res) {
    ...
    if (!validate.test(id)) return res.status(404).json({mensagem: "Formato de ID incorreto."})
    try {
      const carroID = await Carro.findByPk(id)
      if(!carroID) return res.status(404).json({mensagem: "Carro de ID inexistente."})
  ...
  static async getAllCarro (req, res) {
    try {
      ...
      if (!listaCarros.length) return res.json({mensagem: "Não há carros registrados no sistema."})
  ...
  static async patchCarro (req, res) {
    ...
    if (!validate.test(id)) return res.status(404).json( { mensagem: "Formato de ID incorreto." } )
    try {
      if(! await Carro.findByPk(id)) return res.status(404).json( { mensagem: "Carro de ID inexistente." } )
